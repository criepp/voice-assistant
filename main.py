...
def setup_assistant_voice():
    ...
    if assistant.speech_language == "en":
        ...
        if assistant.sex == "female":
            ...
            synthes.speaker = "aidar"
        else:
            # Microsoft David Desktop - English (United States)
            synthes.speaker = "kseniya"
    else:
...
def record_and_recognize_audio(*args: tuple):
    ...
    with microphone:
        ...
        try:
            print("Listening...")
            audio = recognizer.listen(microphone, 15, 15)
...
def play_voice_assistant_speech(text_to_speech):
    ...
    """
    synthes.tts(text_to_speech)
    # pyttsx3.speak(text_to_speech)
...
def get_translation(*args: tuple):
    ...
    search_term = " ".join(args[0])
    translator = TranslateTranslator(to_lang=person.native_language)
    ...
    try:
        ...
        if assistant.speech_language != person.native_language:
            translation_result = translator.translate(search_term, dest=person.native_language)

            play_voice_assistant_speech(f"The translation for {search_term} in Russian is")
        ...
        # если язык речи ассистента и родной язык пользователя одинаковы, то перевод выполняется на изучаемый язык
            ...
            translation_result = translator.translate(search_term, dest=person.target_language)

            play_voice_assistant_speech(f"По-английски {search_term} будет как")
        ...
        play_voice_assistant_speech(translation_result)

    except Exception as e:
        play_voice_assistant_speech(f"Seems like we have a trouble. See logs for more information. Error: {e}")
        traceback.print_exc()
...
api_key="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"
...
def get_answer(prompt):
     response = gpt.response(prompt=prompt)
     print('AI: ', response)
...
def execute_command_with_name(command_name: str, *args: list):
    ...
    if keyscount == 12:
        if command_name != "":
            get_answer(f"{command_name} {args}")
...
commands = {
    ...
    ("shutdown", "turn off", "выключи", "выключение"): shutdown,
}
